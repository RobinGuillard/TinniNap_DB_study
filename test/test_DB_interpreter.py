...
from DB_interpreter import split_database_col, get_mean_std_num_cols, merging_groups
...
def test_merging_groups():
    ...
    myDf = merging_groups(myDf, "A", {"3" : 2})
    print(myDf)
    print(myDf["A"].unique())
    assert len(myDf["A"].unique()) == 2
    assert myDf["A"].unique()[0] == 1
    assert myDf["A"].unique()[1] == 2
